...
const prepStops = stops => {
  const base = (stops && stops.length ? stops : ['#69F0EE', '#34A6CB']).slice(0, MAX_COLORS);
...
export const GradientBlinds = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('[GradientBlinds] WebGL initialization fallback:', e);
      return;
    }
...
`;
    ...
    // Slow, subtle animation loop (0.0004 factor for premium slow movement)
    const loop = t => {
      rafRef.current = requestAnimationFrame(loop);
      uniforms.iTime.value = t * 0.0004;
      ...
      if (!isPaused && programRef.current && meshRef.current) {
        try {
          ...
          // ignore render aborts during teardown
        }
      }
    };
    rafRef.current = requestAnimationFrame(loop);
